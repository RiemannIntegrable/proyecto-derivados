...
\begin{frame}{Métricas de Precisión}
    ● ● \begin{itemize}
        ...
        \item \textbf{MAPE}:
        {\footnotesize $$MAPE = \frac{100}{n}\sum_{i=1}^{n}\left|\frac{y_i -
    \hat{y}_i}{y_i}\right|$$}
  
        \item \textbf{R²}:
        {\footnotesize $$R^2 = 1 - \frac{SS_{res}}{SS_{tot}} = 1 - \frac{\sum_{i=1}^{n}(y_i
    - \hat{y}_i)^2}{\sum_{i=1}^{n}(y_i - \bar{y})^2}$$}
  
        \item \textbf{Correlación}:
        {\footnotesize $$r = \frac{\sum_{i=1}^{n}(x_i - \bar{x})(y_i -
    \bar{y})}{\sqrt{\sum_{i=1}^{n}(x_i - \bar{x})^2 \sum_{i=1}^{n}(y_i - \bar{y})^2}}$$}
